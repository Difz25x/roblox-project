...
function Lonum:CreateFloatingHUD(options)
	...
	HUDFrame.Size = UDim2.new(0, 420, 0, 80)
	HUDFrame.Position = UDim2.new(1, -440, 0, 20)
	...
	function HUDObj:UpdateText(newText)
		...
		for i, line in ipairs(lines) do
			local keyStr = ""
			local valStr = ""
			local colonIdx = string.find(line, ":")
			if colonIdx then
				keyStr = string.sub(line, 1, colonIdx - 1)
				valStr = string.sub(line, colonIdx + 1)
			else
				valStr = line
			end
	...
	function WindowObj:CreateTab(tabName)
		...
		TabButton.Activated:Connect(function()
			for _, t in pairs(self.Tabs) do
				...
				TweenService
					:Create(
						t.Button,
						TweenInfo.new(0.2),
						{ TextColor3 = Lonum.Theme.TextDim, BackgroundTransparency = 1 }
					)
					:Play()
			...
			TweenService
				:Create(
					TabButton,
					TweenInfo.new(0.2),
					{ TextColor3 = Lonum.Theme.TextTitle, BackgroundTransparency = 0 }
				)
				:Play()
		...
		function TabObj:CreateSplitView(options)
			...
			for i = 1, columns do
				...
				function ColObj:CreateCard(options)
					...
					function CardObj:SetContent(newText)
						contentLbl.Text = newText
					end
					function CardObj:SetTitle(newText)
		...
		function TabObj:CreateButton(options)
			...
			btn.MouseEnter:Connect(function()
				...
				TweenService:Create(btnStroke, TweenInfo.new(0.2), { Color = Lonum.Theme.Accent, Transparency = 0 })
					:Play()
			end)
			btn.MouseLeave:Connect(function()
				TweenService:Create(btn, TweenInfo.new(0.2), { BackgroundColor3 = Lonum.Theme.ElementBackground })
					:Play()
				TweenService
					:Create(
						btnStroke,
				...
				for _, opt in ipairs(newOpts or {}) do
					...
					oBtn.MouseEnter:Connect(function()
						TweenService:Create(
							oBtn,
							TweenInfo.new(0.2),
							{ BackgroundColor3 = Lonum.Theme.SidebarBackground, TextColor3 = Lonum.Theme.TextNormal }
						):Play()
					end)
					oBtn.MouseLeave:Connect(function()
						TweenService:Create(
							oBtn,
							TweenInfo.new(0.2),
							{ BackgroundColor3 = Lonum.Theme.ElementBackground, TextColor3 = Lonum.Theme.TextDim }
						):Play()
					end)
		...
		function TabObj:CreateKeybind(options)
			...
			kBtn.Activated:Connect(function()
				...
				isListening = true
				kBtn.Text = "..."
				TweenService:Create(kBtn, TweenInfo.new(0.2), { BackgroundColor3 = Lonum.Theme.SidebarBackground })
					:Play()
